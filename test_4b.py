...
if __name__ == "__main__":
    ...
    parameter = h(true_opt).transpose()[0]
    
    def f(X):
        #fX = np.zeros((m, X.shape[0]))
        hX = h(scale_domain(X))
        aux = (hX.transpose() - parameter).transpose()
        fX = -np.sum(np.square(aux), axis=0)
        return fX
    ...
    max_iter = 100
    for i in range(1):
        filename = './experiments_laptop/test8_random_f_noiseless_' + str(i) + '.txt'
